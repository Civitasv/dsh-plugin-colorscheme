#!/usr/bin/env bash
#
# dsh-plugin-colorscheme — 一键安装脚本（macOS / Linux）
#
# 用法：  bash install.sh
# 或：    ./install.sh
#
...
echo "    验证：  dsh --profile web --dump-config | grep dsh-plugin-colorscheme"
echo "    重启后打开 设置 → 插件 → 插件配置 → 配色方案 卡片即可切换主题。"
